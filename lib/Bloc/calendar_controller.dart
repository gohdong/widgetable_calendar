...
class WidgetableCalendarController extends WidgetableCalendarBloc {
  ...
  void init({
    ...
    super.data.events = events?? [];
    super.data.holidays = holidays ?? [];

    final now = DateTime.now();
    super.data.selectDate = initialDay??_normalizeDate(now);
    super.data.focusDate = initialDay??_normalizeDate(now);
  ...
  List findEvents() {
    ...
    for (int i = 0; i < super.data.events.length; i++) {
      ...
      if (eachEvent.containsKey(super.data.selectDate))
        returnValue.add(eachEvent);
    }
    return returnValue;
  }
  ...
  List _makeWeekList(DateTime firstDay, DateTime lastDay) {
    ...
    for (int i = 1; lastDayWeekday != 6; i++, lastDayWeekday++)
      dateList.add(i + lastDay.day);
