...
class WidgetableCalendarController extends WidgetableCalendarBloc {
  ...
  void init(
      {CalendarFormat calendarFormat, Map holidayData, bool headerEnable}) {
...
//    print(eventData);
  ...
  void deleteEvents(String eventKey, String type) {
    List dateList = [];
    DateTime roundDown(DateTime date) =>
        DateTime(date.year, date.month, date.day);
    Map tempMap = {};
  ...
  Map associatedEventsByDate(DateTime date) {
    ...
    super.data.eachHoliday.forEach((key, value) {
      ...
      if (value['end'].hour != 0 || value['end'].minute != 0) {
            ...
            .subtract(Duration(
                hours: value['end'].hour, minutes: value['end'].minute))
            .add(Duration(days: 1));
        tempValue['end'] = end;
      }
